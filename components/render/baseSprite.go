...
func loadBaseSprite(as fs.FS, dirPath, filenamePrefix string, fps int, resolution c.Vec2) (BaseSprite, error) {
	...
	return BaseSprite{
		SpriteAnimator: c.NewSpriteAnimator(fps, numFrames),
		Rows:           rows,
		Columns:        columns,
...
func (s *BaseSprite) DrawFlipped(x, y float32) {
	srcRec := s.GetRectForFrame(s.GetCurrentFrame())
	...
	rl.DrawTexturePro(s.Resource,
		srcRec,
		rl.NewRectangle(x+s.Resolution.X, y, x-s.Resolution.X, s.Resolution.Y),
		c.V2Z.R(),
		0,
		rl.White,
	)
